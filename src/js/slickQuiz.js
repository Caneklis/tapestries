...
(function ($) {
  $.slickQuiz = function (element, options) {
    var plugin = this,
      ...
      defaults = {
        checkAnswerText: "Проверить!",
        nextQuestionText: "Далее &raquo;",
        questionCountText: "Вопрос %current из %total",
        preventUnansweredText: "Нужно выбрать хотя бы один вариант ответа!",
        nameTemplateText: "<span>Квест: </span>%name",
        disableScore: true,
        disableRanking: true,
    ...
    if (options && typeof options.disableNext != "undefined") {
      ...
      depMsg +=
        "The 'disableNext' option has been deprecated, please use 'preventUnanswered' in it's place.\n\n";
    }
    ...
    if (options && typeof options.disableResponseMessaging != "undefined") {
      ...
      depMsg +=
        "The 'disableResponseMessaging' option has been deprecated, please use" +
    ...
    if (options && typeof options.randomSort != "undefined") {
      ...
      depMsg +=
        "The 'randomSort' option has been deprecated, please use" +
    ...
    var quizValues = plugin.config.json
      ? plugin.config.json
      : typeof quizJSON != "undefined"
      ? quizJSON
      : null;
    ...
    plugin.method = {
      ...
      setupQuiz: function (options) {
        ...
        $quizName
          .hide()
          .html(
            plugin.config.nameTemplateText.replace(
              "%name",
              quizValues.info.name
            )
          )
          .fadeIn(1000, kN(key, 1));
        $quizHeader
          .hide()
          .prepend(
            $('<div class="quizDescription">' + quizValues.info.main + "</div>")
          )
        ...
        for (i in questions) {
          if (questions.hasOwnProperty(i)) {
              ...
              questionHTML.append(
                '<div class="' +
                  ...
                  plugin.config.questionCountText
                    ...
                    .replace(
                      "%total",
                      '<span class="total">' + questionCount + "</span>"
                    ) +
                  "</div>"
              );
            }
            ...
            var selectAny = question.select_any ? question.select_any : false,
              forceCheckbox = question.force_checkbox
                ? question.force_checkbox
                : false,
            ...
            for (i in answers) {
              if (answers.hasOwnProperty(i)) {
                ...
                var optionLabel =
                  '<label for="' + optionId + '">' + answer.option + "</label>";
            ...
            if (
              plugin.config.perQuestionResponseMessaging ||
        ...
        internal.method.turnKeyAndGo(
          key,
          options && options.callback ? options.callback : function () {}
        );
      },
      ...
      resetQuiz: function (startButton, options) {
        ...
        $quizResults.fadeOut(300, function () {
          ...
          plugin.method.startQuiz(
            { callback: plugin.config.animationCallbacks.startQuiz },
            $quizResults
          ); // TODO: determine why $quizResults is being passed
        });
      ...
      checkAnswer: function (checkButton, options) {
        ...
        var questionLI = $($(checkButton).parents(_question)[0]),
          ...
          questionIndex = parseInt(
            questionLI.attr("id").replace(/(question)/, ""),
            10
          ),
          answers = questions[questionIndex].a,
          selectAny = questions[questionIndex].select_any
            ? questions[questionIndex].select_any
            : false;
        ...
        answerSelects.each(function () {
          ...
          selectedAnswers.push(
            parseInt(id.replace(/(.*\_question\d{1,}_)/, ""), 10)
          );
        });
        ...
        var correctResponse = plugin.method.compareAnswers(
          trueAnswers,
          selectedAnswers,
          selectAny
        );
        ...
        questionLI
          .find(correctResponse ? _correctResponse : _incorrectResponse)
          .show();
        ...
        if (plugin.config.perQuestionResponseMessaging) {
          ...
          if (!questionLI.find(_prevQuestionBtn).length)
            kN(key, 2).apply(null, []); // 2nd notch on key must be passed even if there's no "back" button
        } else {
      ...
      completeQuiz: function (options) {
        ...
        if (plugin.config.disableScore) {
          ...
          $(_quizScore + " span").html(
            plugin.config.scoreTemplateText
      ...
      compareAnswers: function (trueAnswers, selectedAnswers, selectAny) {
        if (selectAny) {
          ...
          return (
            $(trueAnswers).not(selectedAnswers).length === 0 &&
